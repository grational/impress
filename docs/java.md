...
This compatibility branch targets Java 8 and Groovy 3. It supports Java POJOs
and map-like items. Java records are not available on this branch.
...
profile.put("skills", Arrays.asList("Java", "DynamoDB"));
...
List<DynamoMapper> entries = Arrays.asList(
...
```java
List<User> users = dynamo
	.query("users", KeyFilter.of("accountId", "account-1"))
	.returning(User.class)
	.backward()
	.take(20)
	.list();
